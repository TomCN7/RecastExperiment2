						 ...
						 const dtMeshTile* tile, unsigned char flags)
{
		...
		for (int i = 0; i < tile->pHeader->nPolyCount; ++i)
		{
			...
			for (unsigned int k = p->uFirstLink; k != DT_NULL_LINK; k = tile->pLinks[k].next)
			{
				if (tile->pLinks[k].edge == 0)
					startSet = true;
				if (tile->pLinks[k].edge == 1)
					endSet = true;
			}
			
			// End points and their on-mesh locations.
			dd->vertex(va[0],va[1],va[2], col);
			dd->vertex(con->fPosition[0],con->fPosition[1],con->fPosition[2], col);
			col2 = startSet ? col : duRGBA(220,32,16,196);
			duAppendCircle(dd, con->fPosition[0],con->fPosition[1]+0.1f,con->fPosition[2], con->fRadius, col2);

			dd->vertex(vb[0],vb[1],vb[2], col);
			dd->vertex(con->fPosition[3],con->fPosition[4],con->fPosition[5], col);
			col2 = endSet ? col : duRGBA(220,32,16,196);
			duAppendCircle(dd, con->fPosition[3],con->fPosition[4]+0.1f,con->fPosition[5], con->fRadius, col2);
			
			// End point vertices.
			dd->vertex(con->fPosition[0],con->fPosition[1],con->fPosition[2], duRGBA(0,48,64,196));
			dd->vertex(con->fPosition[0],con->fPosition[1]+0.2f,con->fPosition[2], duRGBA(0,48,64,196));
			
			dd->vertex(con->fPosition[3],con->fPosition[4],con->fPosition[5], duRGBA(0,48,64,196));
			dd->vertex(con->fPosition[3],con->fPosition[4]+0.2f,con->fPosition[5], duRGBA(0,48,64,196));
			
			// Connection arc.
			duAppendArc(dd, con->fPosition[0],con->fPosition[1],con->fPosition[2], con->fPosition[3],con->fPosition[4],con->fPosition[5], 0.25f,
						(con->uFlags & 1) ? 0.6f : 0, 0.6f, col);
		}
		dd->end();
	}
	...
	for (int i = 0; i < tile->pHeader->nVertCount; ++i)
	{
		const float* v = &tile->fVerts[i*3];
...
{
	// Draw BV nodes.
	const float cs = 1.0f / tile->pHeader->fBoundingVolumeQuantFactor;
	dd->begin(DU_DRAW_LINES, 1.0f);
	for (int i = 0; i < tile->pHeader->nBoundingVolumeNodeCount; ++i)
	{
		const dtBVNode* n = &tile->pBoundingVolumeTree[i];
		if (n->i < 0) // Leaf indices are positive.
			continue;
		duAppendBoxWire(dd, tile->pHeader->fBMin[0] + n->bmin[0]*cs,
						tile->pHeader->fBMin[1] + n->bmin[1]*cs,
						tile->pHeader->fBMin[2] + n->bmin[2]*cs,
						tile->pHeader->fBMin[0] + n->bmax[0]*cs,
						tile->pHeader->fBMin[1] + n->bmax[1]*cs,
						tile->pHeader->fBMin[2] + n->bmax[2]*cs,
...
static void drawMeshTilePortal(duDebugDraw* dd, const dtMeshTile* tile)
{
	...
	const float pady = tile->pHeader->fWalkableClimb;
...
void duDebugDrawNavMeshPoly(duDebugDraw* dd, const dtNavMesh& mesh, dtPolyRef Ref, const unsigned int col)
{
	...
	{
		dtOffMeshConnection* con = &tile->pOffMeshCons[ip - tile->pHeader->nOffMeshBase];

		dd->begin(DU_DRAW_LINES, 2.0f);

		// Connection arc.
		duAppendArc(dd, con->fPosition[0],con->fPosition[1],con->fPosition[2], con->fPosition[3],con->fPosition[4],con->fPosition[5], 0.25f,
					(con->uFlags & 1) ? 0.6f : 0, 0.6f, c);
...
static void debugDrawTileCachePortals(struct duDebugDraw* dd, const dtTileCacheLayer& layer, const float cs, const float ch)
{
	...
	const float* bmin = layer.pHeader->fBMin;
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			{
				if (layer.pConnections[idx] & (1<<(dir+4)))
				{
...
void duDebugDrawTileCacheLayerAreas(struct duDebugDraw* dd, const dtTileCacheLayer& layer, const float cs, const float ch)
{
	...
	const int h = (int)layer.pHeader->cHeight;
	const float* bmin = layer.pHeader->fBMin;
	const float* bmax = layer.pHeader->fBMax;
	const int idx = layer.pHeader->nTileLayer;
	
	unsigned int color = duIntToCol(idx+1, 255);
	
	// Layer bounds
	float lbmin[3], lbmax[3];
	lbmin[0] = bmin[0] + layer.pHeader->cMinX*cs;
	lbmin[1] = bmin[1];
	lbmin[2] = bmin[2] + layer.pHeader->cMinY*cs;
	lbmax[0] = bmin[0] + (layer.pHeader->cMaxX+1)*cs;
	lbmax[1] = bmax[1];
	lbmax[2] = bmin[2] + (layer.pHeader->cMaxY+1)*cs;
...
void duDebugDrawTileCacheLayerRegions(struct duDebugDraw* dd, const dtTileCacheLayer& layer, const float cs, const float ch)
{
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			const int lh = (int)layer.pHeights[lidx];
			if (lh == 0xff) continue;
			const unsigned char reg = layer.pRegions[lidx];
								  ...
								  const float* orig, const float cs, const float ch)
{
	...
	{
		const dtTileCacheContour& c = lcset.pContours[i];
		...
		for (int j = 0; j < c.nVerts; ++j)
		{
			...
			const unsigned char* va = &c.pVerts[j*4];
			const unsigned char* vb = &c.pVerts[k*4];
								  ...
								  const float* orig, const float cs, const float ch)
{
	...
	const int nvp = lmesh.nMaxVertNumPerPoly;
	...
	{
		const unsigned short* p = &lmesh.pPolys[i*nvp*2];
		
		unsigned int color;
		if (lmesh.pAreaIDs[i] == DT_TILECACHE_WALKABLE_AREA)
			color = duRGBA(0,192,255,64);
		else if (lmesh.pAreaIDs[i] == DT_TILECACHE_NULL_AREA)
			color = duRGBA(0,0,0,64);
		else
			color = duIntToCol(lmesh.pAreaIDs[i], 255);
		...
		for (int j = 2; j < nvp; ++j)
		{
			...
			for (int k = 0; k < 3; ++k)
			{
				const unsigned short* v = &lmesh.pVerts[vi[k]*3];
	...
	for (int i = 0; i < lmesh.npolys; ++i)
	{
		...
		for (int j = 0; j < nvp; ++j)
		{
			...
			if ((p[nvp+j] & 0xf) != 0xf)
			{
				const unsigned short* va = &lmesh.pVerts[vi[0]*3];
				const unsigned short* vb = &lmesh.pVerts[vi[1]*3];
	...
	{
		const unsigned short* v = &lmesh.pVerts[i*3];
		const float x = orig[0] + v[0]*cs;
		const float y = orig[1] + (v[1]+1)*ch + 0.1f;
		const float z = orig[2] + v[2]*cs;
